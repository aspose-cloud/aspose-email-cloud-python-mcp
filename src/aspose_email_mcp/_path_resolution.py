...
Sprint 11, extended again to product #4+ by Sprint 12).

Governs *local disk* access only: `email_convert_file`'s local input read (and its converted-
result local output write) and `email_extract_properties`'s local input read are the two Email tools
that touch the local filesystem — both are local-file shapes (formData upload), never a Cloud Storage
round trip.
